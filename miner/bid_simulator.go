	...
	for {
		...
		case newBid := <-b.newBidCh:
			...
			if bestBidToRun != nil {
				...
				if bidRuntime.isExpectedBetterThanSimulatingBid(bestBidRuntime) {
					...
					replyErr = fmt.Errorf("bid is discarded, current best is %s [after BEP95]", bidRuntime.expectedRewardFromBuilder())
					...
					toCommit = false
					replyErr = fmt.Errorf("bid is discarded, current best is %s [after BEP95]", bestBidRuntime.expectedRewardFromBuilder())
...
func (b *bidSimulator) simBid(interruptCh chan int32, bidRuntime *BidRuntime) {
	...
	for _, tx := range bidRuntime.bid.Txs {
		...
		bidRuntime.checkValidatorBribe(b.config.ValidatorBidFeeEOA, tx, receipt)
	}
...
func calcRewardAfterBEP95(preBEP95 *big.Int) *big.Int {
	return new(big.Int).Div(new(big.Int).Mul(preBEP95, big.NewInt(90)), big.NewInt(100))
}
